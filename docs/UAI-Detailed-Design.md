...
上游依据文档：`UAI-REQ-001`（需求定义书 v0.2）、`UAI-BD-001`（基本设计书 v0.2）。本文档设计项以 `UAI-DD-*` 编号，并在 §11 追踪矩阵中与上游 `UAI-BD-*` / `UAI-FR-*` 对应。
...
| UAI-BD-001 | 《Universal Articulation Intelligence 基本设计书》v0.2 | 直接上游依据 |
| UAI-DD-001 | 本文档 | 自身 |
...
| 概念实体命名跨文档一致性 | ⚠ 发现不一致，已修复 | 基本设计 §6.1/§6.2 原用 `ValidationRecord`，详细设计 §3.4/§4/§5.2 统一使用 `ValidationReport`；已将基本设计改为 `ValidationReport` 以对齐详细设计（详见基本设计书 v0.2 改订履历） |
...
**新增 Open Issue**：
- OI-06：DCC 通用集成设计（§11.5）当前仅存在于详细设计文档，尚未回填至需求定义书（`UAI-FR-DCC-*`）与基本设计书（拟编号 `UAI-BD-ARC-015`，须避开已占用的 `UAI-BD-ARC-014`），建议下一版本迭代时补齐三级文档的正式编号与验收标准，并将其 MVP/V1/Research 分级正式写入需求文档 §18/§19 分级表。
